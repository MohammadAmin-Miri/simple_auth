...
class SignupFlowTestCase(APITestCase):
    ...
    def test_signup_user(self):
        ...
        response = self.client.post(reverse(self.VERIFY_PHONE_ENDPOINT), self.req_data)
        ...
        response = self.client.post(reverse(self.VERIFY_EMAIL_ENDPOINT), self.req_data)
        ...
        response = self.client.post(reverse(self.SIGNIN_ENDPOINT), self.req_data)
        self.assertEqual(response.status_code, 201)
...
class UserDetailFlowTestcase(APITestCase):
    ...
    def test_signin_user(self):
        ...
        response = self.client.patch(reverse(self.USER_DETAIL_ENDPOINT), self.req_data)
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.put(reverse(self.USER_CHANGE_PASSWORD_ENDPOINT), self.req_data)
        token = response.data.get('token')
        self.assertEqual(response.status_code, 200)
...
class ResendCodeTestCase(APITestCase):
    ...
    def test_resend_code(self):
        ...
        response = self.client.patch(reverse(self.USER_DETAIL_ENDPOINT), self.new_user_data)
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.post(reverse(self.RESEND_PHONE_ENDPOINT), self.req_data)
        print(response.data)
        self.assertEqual(response.status_code, 201)
        ...
        response = self.client.post(reverse(self.RESEND_EMAIL_ENDPOINT), self.req_data)
        self.assertEqual(response.status_code, 201)
